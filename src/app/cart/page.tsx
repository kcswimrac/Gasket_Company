        ...
        <div className="max-w-4xl mx-auto px-5 sm:px-6 lg:px-8">
          ...
          {items.length === 0 ? (
            <div className="bg-charcoal-900 border border-charcoal-800/50 rounded-2xl p-16 text-center">
              ...
              <p className="text-sm text-charcoal-300 mb-6">Browse the catalog to find the parts you need.</p>
            ...
            <div className="space-y-4">
                ...
                <div key={item.id} className="bg-charcoal-900 border border-charcoal-800/50 rounded-xl p-5 flex flex-col sm:flex-row items-start gap-5">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1.5">
                      ...
                      <span className="text-[10px] text-charcoal-300">{item.material}</span>
                      <span className="text-[10px] text-charcoal-300">{item.process}</span>
                    </div>
                    {item.leadTimeDays && (
                      <p className="text-[10px] text-charcoal-300 mt-1">{item.leadTimeDays} day lead time</p>
                    )}
                    {item.isEstimate && (
                    ...
                    <div className="text-right shrink-0 w-24 ml-auto sm:ml-0">
                      {item.unitPrice ? (
                        <>
                          ...
                          <p className="text-[10px] text-charcoal-300">${item.unitPrice}/ea</p>
                        </>
                      ) : (
                        <p className="text-xs text-charcoal-300">TBD</p>
                      )}
                    </div>

                    {/* Remove */}
                    <button onClick={() => removeItem(item.id)} className="text-charcoal-400 hover:text-red-400 p-1 transition-colors shrink-0">
              ...
              <div className="bg-charcoal-900 border border-charcoal-800/50 rounded-xl p-6 mt-6">
                ...
                <a href="/catalog" className="block text-center text-xs text-charcoal-300 hover:text-charcoal-200 mt-3 uppercase tracking-wider transition-colors">
                  Continue Shopping
                </a>
              </div>
            </div>
